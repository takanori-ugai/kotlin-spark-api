...
import org.apache.spark.sql.catalyst.JavaTypeInference.inferDataType
import org.apache.spark.sql.catalyst.analysis.{GetColumnByOrdinal, UnresolvedAttribute, UnresolvedExtractValue}
import org.apache.spark.sql.catalyst.expressions.objects._
import org.apache.spark.sql.catalyst.expressions.{Expression, _}
import org.apache.spark.sql.catalyst.util._
import org.apache.spark.sql.catalyst.{InternalRow, ScalaReflection, expressions}
...
private[sql] object KotlinSubtypeLock
...
object KotlinReflection extends KotlinReflection {
  ...
  private[sql] def isSubtype(tpe1: `Type`, tpe2: `Type`): Boolean = {
    KotlinSubtypeLock.synchronized {
      tpe1 <:< tpe2
    }
  }
  ...
  private def deserializerFor(
                               ...
                               predefinedDt: Option[DataTypeWithClass] = None): Expression = cleanUpReflectionObjects {
          ...
          case wrapper: KDataTypeWrapper =>
            ...
            val arguments = structType
              ...
              .map(field => {
                ...
                val newTypePath = s"""- field (class: "$clsName", name: "${field.name}")""" +: walkedTypePath

                val newPath = deserializerFor(
                  getType(dataType.cls),
                  Some(addToPath(field.name, dataType.dt, newTypePath)),
                  newTypePath,
                  Some(dataType).filter(_.isInstanceOf[ComplexWrapper])
                )
                if (!nullable) {
                  AssertNotNull(newPath, newTypePath)
                } else {
                  newPath
                }
          ...
          case t: ComplexWrapper =>
            t.dt match {
              case MapType(kt, vt, _) =>
                ...
                val keyPath = s"""- field (key, class: "$classNameForKey")""" +: walkedTypePath
                val valuePath = s"""- field (key, class: "$classNameForValue")""" +: walkedTypePath
                val keyData =
                  Invoke(
                    MapObjects(
                      p => deserializerFor(
                      ...
                      Invoke(getPath, "keyArray", ArrayType(keyDT.dt)),
                      keyDT.dt),
                    "array",
                ...
                val valueData =
                  Invoke(
                    MapObjects(
                      p => deserializerFor(valueType, Some(p), valuePath, Some(valueDT).filter(_.isInstanceOf[ComplexWrapper])),
                      Invoke(getPath, "valueArray", ArrayType(valueDT.dt)),
                      keyDT.dt),
                    "array",
                    ObjectType(classOf[Array[Any]]))
              ...
              case ArrayType(elementType, containsNull) =>
                ...
                val elementClass = dataTypeWithClass.cls
                val newTypePath = s"""- array element class: "${getClassFromType(getType(elementClass))}"""" +: walkedTypePath

                val primitiveMethod = elementClass match {
                ...
                primitiveMethod.map { method =>
                  Invoke(getPath, method, ObjectType(elementClass))
                }.getOrElse {
                  Invoke(
                    MapObjects(
                      p => deserializerFor(getType(elementClass), Some(p), newTypePath, Some(dataTypeWithClass).filter(_.isInstanceOf[ComplexWrapper])),
                      getPath,
                      dataTypeWithClass.dt),
                    "array",
                    ObjectType(elementClass))
                }
